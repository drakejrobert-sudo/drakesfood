...
- Open a non-draft pull request linked to the issue when work is ready for review unless Drake explicitly asks for a draft PR.
...
PRs should be opened ready for review by default, not as drafts. Use a draft PR only when Drake explicitly requests one.
